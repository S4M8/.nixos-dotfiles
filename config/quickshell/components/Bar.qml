...
PanelWindow {
	...
	SequentialAnimation {
		...
		NumberAnimation { 
			target: bar; property: "chargingPulse"; 
			to: 0.3; 
			duration: 800; 
	...
	SequentialAnimation {
		...
		running: bar.batteryCharging
        	loops: Animation.Infinite
		NumberAnimation { 
			target: bar; property: "chargingFillAnim"; 
			to: 1; 
			duration: 2000; 
			easing.type: Easing.InOutSine 
		}
 ...
 component Notch: Item {
    ...
    Rectangle {
        ...
        color: notchRoot.hovered ? bar.notchHoverColor : bar.notchColor
        Behavior on color {
            ...
            Row {
                ...
                Timer {
                    ...
                    onTriggered: {
                        ...
                        for (var wsNum = 1; wsNum <= 3; wsNum++)
                            ids.add(wsNum)
                        var sorted = Array.from(ids).sort(function(a, b) { return a - b })
                        wsModel.clear()
                        for (var j = 0; j < sorted.length; j++)
                            wsModel.append({ wsId: sorted[j] })
                    }
                }
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        readonly property bool isActive: Hyprland.focusedWorkspace
                            ? Hyprland.focusedWorkspace.id === model.wsId
                            : false
                        readonly property bool hasWindows: {
                            var found = false
                            var vals = Hyprland.workspaces.values
                            for (var i = 0; i < vals.length; i++) {
                                if (vals[i].id === model.wsId) { found = true; break }
                            }
                            return found
                        }
						...
						Item {
							...
							Rectangle {
								...
								Rectangle {
									...
									Behavior on width { 
										NumberAnimation { 
											duration: bar.batteryCharging ? 50 : 600; 
											easing.type: Easing.OutCubic 
										} 
									}
									Behavior on color { 
