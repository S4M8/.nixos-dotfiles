			...
			ColumnLayout {
				...
				Rectangle {
					...
					Row {
						...
						PowerBtn { 
							icon: "⏻"; 
							iconColor: root.walColor2; 
							cmd: "systemctl poweroff" 
						}
						PowerBtn { 
							icon: "󰜉"; 
							iconColor: root.walColor13; 
							cmd: "systemctl reboot" 
						}
						PowerBtn { 
							icon: "󰌾"; 
							iconColor: root.walColor5; 
							cmd: "hyprlock" 
						}
						PowerBtn { 
							icon: "󰒲"; 
							iconColor: root.walColor4; 
...
Process {
    ...
    Process {
		id: vpnCheckProc
		command: ["bash", "-c", "systemctl is-active wg-quick-surfshark 2>/dev/null | grep -q '^active$' && echo on || echo off"]
        stdout: SplitParser {
...
Process {
    id: vpnNotifyProc
    property string msg: ""
    command: ["bash", "-c", "notify-send -a 'Surfshark' -i network-vpn '" + msg + "'"]
}
    Process {
		id: vpnToggleProc
...
command: {
    if (vpnToggle.vpnEnabled)
        return ["bash", "-c", "sudo systemctl stop wg-quick-surfshark 2>&1"]
    else
        return ["bash", "-c", "sudo systemctl start wg-quick-surfshark 2>&1"]
}
        onExited: {
            vpnCheckTimer.start()
        }
...
Process {
    ...
    stdout: SplitParser {
        onRead: data => {
            ...
            if (isOn) {
                vpnNotifyProc.msg = "VPN Connected — Surfshark is active"
                vpnNotifyProc.running = true
            } else {
                vpnNotifyProc.msg = "VPN Disconnected — Surfshark is off"
                vpnNotifyProc.running = true
            }
        }
    }
...
Process {
    ...
    stdout: SplitParser {
        onRead: data => {
            ...
            if (vpnToggle.vpnEnabled && !isOn) {
                ...
                vpnNotifyProc.msg = "VPN Crashed — Surfshark dropped unexpectedly!"
                vpnNotifyProc.running = true
            }
        }
    }
				...
				Rectangle {
					Layout.fillWidth: true
						...
						anchors.centerIn: parent
							...
							label: "CPU"; 
							icon: ""; 
							barColor: root.walColor1; 
							value: dashboard.cpuVal 
						}
                        			CircularStat { 
							label: "RAM"; 
							icon: ""; 
	...
	Process {
		id: uptimeProc
		command: ["bash", "-c", "awk '{s=int($1); m=s/60; h=s/3600; d=s/86400; mo=s/2592000; if (mo>=1) printf \"up %d month%s %d day%s\", mo, (mo==1?\"\":\"s\"), (d%30), ((d%30)==1?\"\":\"s\"); else if (d>=1) printf \"up %d day%s %d hour%s\", d, (d==1?\"\":\"s\"), (h%24), ((h%24)==1?\"\":\"s\"); else if (h>=1) printf \"up %d hour%s %d min\", h, (h==1?\"\":\"s\"), (m%60); else printf \"up %d min\", m}' /proc/uptime"]
		stdout: SplitParser { 
