...
//@ pragma ShellId nixos-s4m8-shell
import Quickshell
...
ShellRoot {
	...
	function addNotification(nid, app, title, body) {
	...
	Process {
		...
		command: ["bash", "-c",
			...
			"HASH_CACHE=\"$THUMB_DIR/hashes.json\" && " +
			"WALL_DIR='" + root.wallpaperPath + "' && " +
			"mkdir -p \"$THUMB_DIR\" && " +
			"if [ -f \"$HASH_CACHE\" ] && [ \"$(find \"$WALL_DIR\" -maxdepth 1 -type f -newer \"$HASH_CACHE\" 2>/dev/null)\" = \"\" ]; then " +
			"  cat \"$HASH_CACHE\"; exit 0; " +
			"fi && " +
			"echo -n '{' > \"$HASH_CACHE.tmp\" && " +
			"first=true && " +
			...
			"  $first || echo -n ',' >> \"$HASH_CACHE.tmp\"; first=false; " +
			"  echo -n \"\\\"$f\\\":\\\"$hash\\\"\" >> \"$HASH_CACHE.tmp\"; " +
			...
			"done && " +
			"echo -n '}' >> \"$HASH_CACHE.tmp\" && " +
			"mv \"$HASH_CACHE.tmp\" \"$HASH_CACHE\" && cat \"$HASH_CACHE\""
		]
		stdout: SplitParser {
			splitMarker: ""
			onRead: data => {
				try {
					...
					root.wallpaperHashes = json
				} catch(e) {
					root.wallpaperHashes = ({})
				}
			}
		}
	...
	Process {
		...
		stdout: SplitParser {
			...
			onRead: data => {
				try { 
					root.appUsage = JSON.parse(data.trim()) 
				} catch(e) { 
					root.appUsage = {} 
				}
			}
